...
## 4. 주요 기능
### 이미지 클릭 시 동영상 링크로 이동됩니다. 우클릭, 새 탭에서 링크 열기 클릭 시 새 탭으로 동영상이 재생됩니다.
...
### 4.8 차량관리
##### ☀️ 담당기능 (이미지 클릭 시 동영상 링크로 이동됩니다. 우클릭, 새 탭에서 링크 열기 클릭 시 새 탭으로 동영상이 재생됩니다.)
...
1. 차량관리 페이지
 ...
 - 전체 목록을 등록번호 역순으로 출력하고 있습니다. 필터를 통해 원하는 조건으로 조회 가능합니다.

 - 페이징 처리를 통해 10개 단위 조회가 가능합니다.


2. 차량 상세 페이지

 - 차량에 대한 기본적인 정보와 수리 내역을 확인할 수 있습니다. 
 ...
 - 신규차량 등록. 정보를 입력하고 이미지를 선택하면 차량 정보가 새로 등록, 목록이 갱신됩니다.
...
1. 협력사 정보 페이지
 ...
 - 주소창을 클릭하면 카카오 주소 API를 호출합니다. 선택 시 주소 창에 자동 입력됩니다.
...
1. 출장목록 리스트 형식식 페이지
 ...
 - 사용자의 출장 목록을 출력합니다. 하단에 협력사 정보를 제공하여 사용자 의성을 추가하였습니다.


2. 출장 목록을 달력 형식 페이지

 - 탭을 통해 글 형식과 달력 형식을 변경할 수 있습니다.
 
 - 달력 ui는 FullCalendar Api이고 출력된 공휴일 정보는 GoogleCalendar Api에서 정보를 받아와 Redis 서버를 거쳐 전송된 데이터입니다.
...
3. 출장 상세 페이지
 ...
 - 출장 정보에 대한 상세 내역을 조회할 수 있습니다.


4. 출장 신청서 페이지

 - 고정 값으로 사용자의 정보가 자동 입력됩니다. 날짜 유효성 검사가 설정되어 있습니다.

 - 출발지 직접 입력버튼 클릭 시 카카오 주소 API를 호출하여 주소 값을 받아옵니다.

 - 출장지 위치로 협력사 선택을 누르면 모달 창이 출력되며 등록된 협력사 주소 클릭 시 해당 주소 정보를 가져옵니다.

 - 종료 날짜가 시작 날짜를 앞서지 못하도록 유효성 검사가 설정되어 있습니다.


5. 배차 신청서 페이지

 - 추가적인 배차신청여부 버튼입니다. 버튼을 누르지 않으면 배차신청서는 제출되지 않습니다.
...
6. 배차관리 페이지
 ...
 - 관리자는 배차 신청서가 들어오면 관리자가 상세 정보를 조회할 수 있습니다.
...
1. 공휴일 관리 페이지
 ...
 - 사내 캘린더 전체에 반영되는 휴일을 지정할 수 있습니다. 
...
1. 화면제어 방식
 ...
 - Case 1 : 일반 사용자 계정으로 관리시스템에 접근

 - 관리자 권한이 없는 경우, 알림 페이지를 출력 후 메인 화면으로 강제 이동시킵니다.

 - Case 2 : 동일한 페이지를 관리자 계정으로 진입
...
![스크린샷 2024-09-12 123939](https://github.com/user-attachments/assets/c2c5b5b7-6086-4314-9f60-ae98b729536b)




#### 2) 1차 개선: 서버에서 1년치 데이터를 미리 요청하여 **0.9초**로 단축.
![스크린샷 2024-09-12 124718](https://github.com/user-attachments/assets/c3e49479-c35e-42b5-a7a0-53db7b2247bd)
...
![스크린샷 2024-09-12 135052](https://github.com/user-attachments/assets/0972137a-c5dd-461c-8da6-cc6f8bab7dde)
