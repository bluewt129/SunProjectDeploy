...
const VehicleDetailComponent = () => {
  ...
  const getVehicleDetail = async () => {
    try {
      const response = await axios.get(`http://sun-api:8787/api/vehicle/${vehicleCode}`);
  ...
  const getRepairDetail = async () => {
    try {
      const response = await axios.get(`http://sun-api:8787/api/repair/${vehicleCode}`);
  ...
  const getVehicleImages = async () => {
    try {
      const response = await axios.get(`http://sun-api:8787/api/vehicleImages/${vehicleCode}`);
  ...
  const handleSaveClick = async () => {
    ...
    if (image !== null) {
      ...
      try {
        const response = await axios.post("http://sun-api:8787/api/vehicleImg", formImgData, {
          headers: {
    ...
    if (repairsToSave.length > 0) {
      try {
        for (const repairCode of repairsToSave) {
          await axios.post(`http://sun-api:8787/api/updateRepair/${repairCode}`);
        }
        Swal.fire({
          title: "성공!",
          text: "변경이 저장되었습니다.",
  ...
  const deleteVehicle = async () => {
    try {
      await axios.post(`http://sun-api:8787/api/vehicleDel/${vehicleCode}`);
      Swal.fire({
        title: "삭제 완료",
        text: "차량이 삭제되었습니다",
        icon: "success",
