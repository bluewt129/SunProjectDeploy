...
const VehicleRentDetailComponent = ({ vrsvCode, onApproval }) => {
  ...
  const getVehicleRentDetail = async () => {
    try {
      const response = await axios.get(`http://sun-api:8787/api/vrentDetail/${vrsvCode}`);
  ...
  const handleApproval = async () => {
    try {
      await axios.post("http://sun-api:8787/api/approveVRent", null, {
        params: { vrsvCode }
      });
  ...
  const handleRejection = async () => {
    try {
      await axios.post("http://sun-api:8787/api/rejectVRent", null, {
