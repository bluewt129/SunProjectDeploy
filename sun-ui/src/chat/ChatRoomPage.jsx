...
const ChatRoomPage = ({ show, handleClose, chatroomCode ,onUpdateChatRoomList}) => {
    ...
    const fetchMessages = async () => {
        try {
            const response = await fetch(`http://sun-api:8787/chatList?chatroomCode=${chatroomCode}`);
            if (!response.ok) {
    ...
    const fetchParticipants = async () => {
        try {
            const chatResponse = await fetch(`http://sun-api:8787/api/partiList?chatroomCode=${chatroomCode}`);
            if (!chatResponse.ok) {
                throw new Error('Network response was not ok');
            }
    ...
    useEffect(() => {
        const socket = new SockJS('http://sun-api:8787/ws');
