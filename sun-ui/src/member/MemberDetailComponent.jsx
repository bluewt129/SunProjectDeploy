...
const MemberDetailComponent = () => {
    ...
    useEffect(() => {
        const fetchEmployeeData = async () => {
            try {
                const response = await fetch(`http://sun-api:8787/memberDetail/${empCode}`);
                if (!response.ok) {
                    throw new Error('Network response was not ok');
                }
                const data = await response.json();
                
                axios.get(`http://sun-api:8787/getMemberImage?empCode=${empCode}`)
    ...
    const handlePasswordReset = async () => {
        try {
            const response = await fetch(`http://sun-api:8787/resetPassword/${empCode}`, {
                method: 'POST',
                headers: {
