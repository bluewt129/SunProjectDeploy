...
const BoardDetailComponent = () => {
    ...
    useEffect(() => {
        const fetchNoticeData = async () => {
            try {
                // 특정 게시글 가져오기
                const response = await fetch(`http://sun-api:8787/boardDetail/${notiCode}`);
                const data = await response.json();
                setNotice(data);

                // 사원번호 가져오기 및 사원이름 가져오기
                const empResponse = await fetch(`http://sun-api:8787/api/employee/${data.empCode}`);
                if (empResponse.ok) {
                ...
                // 파일 목록 가져오기
                const filesResponse = await fetch(`http://sun-api:8787/files/${notiCode}`);
                if (filesResponse.ok) {
    ...
    const handleDelete = async () => {
        try {
            ...
            const response = await axios({
                url: `http://sun-api:8787/deleteBoard/${notiCode}`,
                method: 'POST',
                params: {
                                ...
                                <ul>
                                    {files.map(file => (
                                        <li key={file.fileName}>
                                            <a href={`http://sun-api:8787/files/download/${file.nfileFakename}`} download>
