...
const BTripFormComponent = () => {
    ...
    useEffect(() => {
        if (sessionEmp) {
            const getUserInfo = async () => {
                try {
                    const response = await axios.get(`http://sun-api:8787/memberDetail/${sessionEmp}`);
    ...
    const handleSubmit = async () => {
        ...
        try {
            const response = await axios.post("http://sun-api:8787/api/insertBTrip", constAllData);
